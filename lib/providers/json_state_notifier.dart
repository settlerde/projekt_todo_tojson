...
class JsonStateNotifier extends AppStateNotifierInterface {
  final _storageService = StorageService();

  @override
  AppState build() {
    //entry point for notifier, called when provider is first accessed.
    Future.microtask(() => loadState()); // a way to trigger an asynchronous load at startup without blocking the UI.
    return AppState();
  }
  ...
  Future<void> deleteSelectedTodos() async {
    ...
    final remainingTodos = state.todos.where((todo) => !state.selectedTodoIds.contains(todo.id)).toList();
  ...
  void toggleDeletionConfirmation() {
    state = state.copyWith(asksForDeletionConfirmation: !state.asksForDeletionConfirmation);
    saveState();
  }
